# Installing Cyclone UI (Light Dimmed)

Generated by `@razorwind/shiki`.

## Files

- `cyclone-light-dimmed.json` — Cyclone Light (theme id: `cyclone-lightDimmed`)

## Setup

1. Copy or import the generated JSON into your app (paths depend on your bundler).

2. Register themes when creating a Shiki highlighter:

```ts
import { createHighlighter } from "shiki";
import theme from "./cyclone-light-dimmed.json";

const highlighter = await createHighlighter({
  themes: [theme],
  langs: ["javascript"]
});

const html = highlighter.codeToHtml("const x = 1", {
  lang: "javascript",
  theme: "cyclone-lightDimmed"
});
```

Or load at runtime:

```ts
import { readFile } from "node:fs/promises";

const theme = JSON.parse(
  await readFile("./cyclone-light-dimmed.json", "utf8")
);
await highlighter.loadTheme(theme);
```

## Select / load theme

Pass the theme id to `codeToHtml`, `codeToTokens`, or `getTokenStyle`:

- `cyclone-lightDimmed` — Cyclone Light
