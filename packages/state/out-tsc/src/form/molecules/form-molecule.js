...
import { atomWithFieldsMessageList, atomWithFieldsMessageTypes, atomWithMessages, atomWithMessageTypes, atomWithTheme } from "../atoms/atom-with-messages";
...
export const FormApi = createMoleculeApi((scope) => {
    const optionsBaseAtom = atomWithReset(DEFAULT_FORM_OPTIONS);
    const optionsAtom = atom(get => get(optionsBaseAtom), (get, set, update) => {
        let nextValue = {};
    ...
    const disabledAtom = atomWithDefault(get => Boolean(get(optionsAtom).disabled));
    const formValidatingAtom = atomWithReset(false);
    const previousValuesBaseAtom = atomWithReset({});
    const previousValuesAtom = atom(get => get(previousValuesBaseAtom), (_get, _set, _update) => { });
    const initialValuesAtom = atomWithDefault(get => get(optionsAtom).initialValues);
    const valuesBaseAtom = atomWithDefault(get => get(optionsAtom).initialValues);
    const valuesAtom = atom(get => get(valuesBaseAtom), (get, set, update) => {
        let nextValue = {};
        if (update) {
            if (isFunction(update)) {
    ...
    const tabIndexesAtom = atom(get => get(tabIndexesBaseAtom), (get, set, update) => {
        let nextValue = {};
        if (isFunction(update)) {
            ...
            if ("field" in update && update.field && isString(update.field)) {
                ...
                nextValue = setField(tabIndexes, update.field, tabIndex);
            }
            else {
                nextValue = update;
            }
        }
        set(tabIndexesBaseAtom, nextValue);
    });
    ...
    const formWarningsAtom = atomWithMessageTypes(validationResultsAtom, "warning");
    ...
    const formSuccessesAtom = atomWithMessageTypes(validationResultsAtom, "success");
    const isFormInvalidAtom = atom(get => {
        const errorMessages = get(formErrorsAtom);
        return errorMessages.length > 0;
    });
    const errorFieldsAtom = atomWithFieldsMessageTypes(validationResultsFieldsAtom, "error");
    const warningFieldsAtom = atomWithFieldsMessageTypes(validationResultsFieldsAtom, "warning");
    const infoFieldsAtom = atomWithFieldsMessageTypes(validationResultsFieldsAtom, "info");
    const helpFieldsAtom = atomWithFieldsMessageTypes(validationResultsFieldsAtom, "help");
    const successFieldsAtom = atomWithFieldsMessageTypes(validationResultsFieldsAtom, "success");
    const fieldErrorMessagesAtom = atomWithFieldsMessageList(validationResultsFieldsAtom, "error");
    const fieldWarningMessagesAtom = atomWithFieldsMessageList(validationResultsFieldsAtom, "warning");
    const fieldInfoMessagesAtom = atomWithFieldsMessageList(validationResultsFieldsAtom, "info");
    const fieldHelpMessagesAtom = atomWithFieldsMessageList(validationResultsFieldsAtom, "help");
    const fieldsSuccessMessagesAtom = atomWithFieldsMessageList(validationResultsFieldsAtom, "success");
    ...
    const canSubmitAtom = atom(get => get(validAtom) &&
        !get(validatingAtom) &&
        !get(submittingAtom) &&
        !get(disabledAtom));
    return {
        ...
        messages: atomWithMessages(formErrorsAtom, formWarningsAtom, formInfoAtom, formHelpAtom, formSuccessesAtom),
        ...
        valid: validAtom,
        invalid: invalidAtom,
        theme: atomWithTheme(optionsAtom, formErrorsAtom, formWarningsAtom, formInfoAtom, formHelpAtom, formSuccessesAtom),
        canSubmit: canSubmitAtom
    };
}, {
    type: "form"
});
//# sourceMappingURL=form-molecule.js.map