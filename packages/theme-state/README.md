...
# Theme state management package
...
- [Theme state management package](#theme-state-management-package)
...
```bash
pnpm add -D @cyclone-ui/theme-state
```
...
```bash
npm install -D @cyclone-ui/theme-state
```
...
```bash
yarn add -D @cyclone-ui/theme-state
```
...
Run `nx build theme-state` to build the library.

### Running unit tests

Run `nx test theme-state` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint theme-state` to run [ESLint](https://eslint.org/) on the package.
