...
export const build = async ({
  ...
  const pkg = fs.readJSONSync(join(project, "./package.json"));
  const pkgMain = pkg.main;
  const pkgSource = project ? project : pkg.source ? pkg.source : process.cwd();
  ...
  async function handleClean() {
    try {
      await Promise.allSettled([
        fs.remove(output ? output : join(project, "output-dir")),
        fs.remove(join(project, ".turbo")),
        fs.remove(join(project, "node_modules")),
        fs.remove(join(project, ".ultra.cache.json")),
        fs.remove(join(project, "types")),
        fs.remove(join(project, "dist"))
      ]);
    } catch {
      // ok
    }
    ...
    try {
      await Promise.allSettled([fs.remove(join(project, "node_modules"))]);
    } catch {
      // ok
    }
    console.info("🔹 cleaned", pkg.name);
  ...
  async function buildTsc() {
    ...
    const targetDir = join(project, "types");
    try {
      // typescripts build cache messes up when doing declarationOnly
      await fs.remove(join(project, "tsconfig.tsbuildinfo"));
      ...
      const tsProjectFlag = tsProject
        ? ` --project ${tsProject ? tsProject : '"./tsconfig.json"'}`
        : "";
      const cmd = `tsc${baseUrlFlag}${tsProjectFlag} --root-dir \"${root}\" --outDir \"${targetDir}\" ${declarationToRootFlag}--emitDeclarationOnly --declarationMap`;

      console.info("\x1b[2m$", `npx ${cmd}`);
      await execa("npx", cmd.split(" "));
    } catch (err) {
  ...
  async function buildJs() {
    ...
    const files = bundle
      ...
      : (
          ...
          x =>
            ...
            !x.match(join(project, ".tamagui/**/*")) &&
            !x.match(join(project, "dist/**/*"))
        );
    ...
    const cjsConfig = {
      format: "cjs",
      entryPoints: files,
      outdir: flatOut ? join(output, "dist") : "dist/cjs",
      bundle,
      external,
