...
/**
 * A console text animation controller returned by functions like `shine`.
 */
export interface TextAnimation {
  /**
   * Replace the animated text while the animation is running.
   */
  replace: (str: string) => TextAnimation;
  /**
   * Stop the animation.
   */
  stop: () => TextAnimation;
  ...
  start: () => TextAnimation;
  /**
   * Advance one frame and return the ANSI-colored output for the current text.
   */
  frame: () => string;
}

let currentTextAnimation: TextAnimation | null = null;

/**
 * Stop the currently running text animation, if any.
 */
function stopLastTextAnimation() {
...
/**
 * Apply one frame of the gray/white shine effect to a string.
 *
 * @remarks
 ...
 * @param str - The text to colorize for the given frame.
 * @param frame - The current animation frame index.
 * @returns The input string with ANSI gray/white shine coloring applied for the
 *   frame.
 *
 */
function applyShineEffect(str: string, frame: number) {
  if (!isColorSupported) {
    return str;
  }
  ...
  return chars
    .map((char, index) => {
      ...
      const angle = ((index / Math.max(length - 1, 1)) * 360 + phase) % 360;
      ...
      return t > 0.6 ? `\x1b[37m${char}\x1b[39m` : `\x1b[90m${char}\x1b[39m`;
    })
    .join("");
}
...
function animateText(
  str: string,
  effect: (str: string, frame: number) => string,
  delay: number,
  speed: number | undefined
) {
  stopLastTextAnimation();

  const resolvedSpeed =
    speed === undefined ? 1 : Number.parseFloat(String(speed));
...
/**
 * Animate text with a gray/white shine effect.
 *
 * @remarks
 ...
 * @param speed - Animation speed multiplier. Must be greater than `0`. Defaults
 *   to `1`.
 * @returns A `TextAnimation` controller with `start`, `stop`, `replace`, and
 *   `frame` methods.
 *
 * @example
 * ```ts
 ...
 * ```
 */
export function shine(text: string, speed?: number) {
  return animateText(text, applyShineEffect, 15, speed);
}
...
export type BorderOption =
  "primary" | "secondary" | "tertiary" | "none" | string;
