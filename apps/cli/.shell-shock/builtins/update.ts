      ...
      if (options.isDependencyRequired) {
        ...
        if (
          Object.keys(packageJson.dependencies || {})
            ...
            .some(
              dep =>
                dep === "@cyclone-ui/cli" || dep.startsWith("@cyclone-ui/cli@")
            )
        ) {
...
export async function getUpdateCommand(
  ...
  const version = (await getLatestVersion("@cyclone-ui/cli")) || "latest";

  if (packageManager === "yarn") {
    return ["upgrade", `@cyclone-ui/cli@${version}`];
  } else if (packageManager === "pnpm") {
    return ["update", `@cyclone-ui/cli@${version}`];
  } else if (packageManager === "deno") {
    return ["outdated", "--update", `@cyclone-ui/cli@${version}`];
  } else if (packageManager === "bun") {
    return ["update", "--save", `@cyclone-ui/cli@${version}`];
  } else {
    return ["update", "--save", "--bin-links", `@cyclone-ui/cli@${version}`];
  }
}
...
export type CheckForUpdatesResult =
  CheckForUpdatesSuccessResult | CheckForUpdatesErrorResult;
...
export async function checkForUpdates(
  ...
  try {
    const pkg = await fetchNpmPackage("@cyclone-ui/cli");

    return {
      latestVersion: pkg?.version || "0.6.1",
      currentVersion: "0.6.1",
      isUpToDate: pkg ? "0.6.1" === pkg.version : true,
      package: pkg,
      isError: false
    };
  } catch (err) {
...
/**
 * Run update processing for the Cyclone User Interface application.
 */
  ...
  if (await isCheckForUpdatesRequired()) {
    ...
    if ((result as CheckForUpdatesErrorResult)?.isError) {
      ...
      try {
        ...
        writeLine("");
        help(
          `The changelog for this release can be viewed by running the ${inlineCode("cyclone-ui changelog")} command.`
        );
        ...
        spinner.error(
          `An error occurred while updating Cyclone User Interface to v${(result as CheckForUpdatesSuccessResult).latestVersion}. Please try again later - if the problem persists, please contact support.`
        );
        debug(err);
      }
    } else {
      spinner.success(
        "Currently running the latest version of Cyclone User Interface."
      );
      writeLine("");
    }
  }
