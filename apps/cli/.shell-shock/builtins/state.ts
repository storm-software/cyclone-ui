...
export function setState(
  ...
  if (!prev) {
    throw new Error(
      `The Powerlines application context is not available. Make sure to call setState() within a valid context scope.`
    );
  }
...
export function useCommand(): CommandContext {
  ...
  if (!result) {
    throw new Error(
      `The Powerlines - command context is not available. Make sure to call useCommand() within a valid context scope.`
    );
  }
  return result;
}
...
export async function withGlobal(handler: () => any): Promise<Promise<void>> {
  ...
  const options = {
    help: false,
    version: false,
    verbose: env.POWERLINES_VERBOSE ?? false,
    color: env.POWERLINES_COLOR ?? false,
    noColor: env.POWERLINES_NO_COLOR ?? true,
    noBanner: env.POWERLINES_NO_BANNER ?? false,
    interactive: env.POWERLINES_INTERACTIVE ?? true,
    nonInteractive: env.POWERLINES_NON_INTERACTIVE ?? false,
    yes: env.POWERLINES_YES ?? false
