...
export default class Build extends Command {
  public static override args = {
    projectRoot: Args.directory({
      ...
      required: true,
      description:
        "The path of the project to build relative to the workspace root",
      ignoreStdin: false,
  ...
  public static override examples = [
    ...
    },
    {
      ...
      command:
        "<%= config.bin %> <%= command.id %> build --output=./path/to/output"
    },
    {
      description:
        "Build the themes, write the output to ./path/to/output, and skip confirmation prompts",
      command:
        "<%= config.bin %> <%= command.id %> build --output=./path/to/output --skip"
    }
  ];
