...
export default class Get extends Command {
  ...
  public override async run(): Promise<void> {
    ...
    if (!library) {
      ...
      if (!flags.skip) {
        ...
        if (!useConfigOutput) {
          const promptInput = await text({
            message: 'Enter the local components library package output directory',
      ...
      if (!library) {
        this.error('The components library package directory is required. Please provide a valid directory path.')
      }
    }
    ...
    if (!registry) {
      ...
      if (!flags.skip) {
        ...
        if (!useConfigOutput && !registry) {
          ...
          registry = promptInput && typeof promptInput === 'string' ? new URL(promptInput) : undefined
        }
      }

      if (!registry) {
        this.error('A remote URL for the Cyclone UI registry is required. Please provide a valid URL.')
      }
    }
    ...
    const s3 = spinner()
    s3.start('Checking for components.json file in local design components library...')

    let localComponents = {}
    if (!(await fs.exists(library))) {
      localComponents = await fs.writeJson(join(library, 'components.json'), {})

      s3.stop('Added components.json file to local design components library...')
    } else {
      localComponents = await fs.readJson(join(library, 'components.json'))
      s3.stop('Found components.json file in local design components library...')
    }
    ...
    if (components && components.length > 0) {
      ...
      if (components) {
        ...
        if (!useConfigOutput) {
          components = await this.componentPrompt(remoteComponents as any, localComponents)
        }
      }
    } else {
      components = await this.componentPrompt(remoteComponents as any, localComponents)
    ...
    if (!components || components.length === 0) {
      this.error('No components were selected to add to the local components library')
    }
  ...
  async componentPrompt(remoteComponents: any[], localComponents: Record<string, string>): Promise<string[]> {
    const promptInput = await multiselect<Option<string>[], string>({
      message: 'Select the components to add to the local components library package',
