...
A `LabelText` component used to render the body text of a page. It is a simple component that renders the text in a way that is easy to read and understand.
...
```bash
pnpm add -D @cyclone-ui/label-text
```
...
```bash
npm install -D @cyclone-ui/label-text
```
...
```bash
yarn add -D @cyclone-ui/label-text
```
...
Run `nx build label-text` to build the library.

### Running unit tests

Run `nx test label-text` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint label-text` to run [ESLint](https://eslint.org/) on the package.
