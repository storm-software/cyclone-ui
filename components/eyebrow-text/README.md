...
```bash
pnpm add -D @cyclone-ui/eyebrow-text
```
...
```bash
npm install -D @cyclone-ui/eyebrow-text
```
...
```bash
yarn add -D @cyclone-ui/eyebrow-text
```
...
Run `nx build eyebrow-text` to build the library.

### Running unit tests

Run `nx test eyebrow-text` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint eyebrow-text` to run [ESLint](https://eslint.org/) on the package.
