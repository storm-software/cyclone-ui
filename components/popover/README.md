...
A package containing the Popover component used by Storm Software
...
```bash
npm install -D @cyclone-ui/popover
```
...
Run `nx build popover` to build the library.

### Running unit tests

Run `nx test popover` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint popover` to run [ESLint](https://eslint.org/) on the package.
