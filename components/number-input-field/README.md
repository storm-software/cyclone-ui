...
Number Input Form Field component for Cyclone UI
...
```bash
pnpm add -D @cyclone-ui/number-input-field
```
...
```bash
npm install -D @cyclone-ui/number-input-field
```
...
```bash
yarn add -D @cyclone-ui/number-input-field
```
...
Run `nx build number-input-field` to build the library.

### Running unit tests

Run `nx test number-input-field` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint number-input-field` to run [ESLint](https://eslint.org/) on the package.
