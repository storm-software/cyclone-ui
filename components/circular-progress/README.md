...
A Progress component meant to be shared in many other components
...
```bash
pnpm add -D @cyclone-ui/circular-progress
```
...
```bash
npm install -D @cyclone-ui/circular-progress
```
...
```bash
yarn add -D @cyclone-ui/circular-progress
```
...
Run `nx build circular-progress` to build the library.

### Running unit tests

Run `nx test circular-progress` to execute the unit tests via
[Vitest](https://vitest.dev).

### Linting

Run `nx lint circular-progress` to run [ESLint](https://eslint.org/) on the package.
