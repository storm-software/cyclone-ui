...
```bash
pnpm add -D @cyclone-ui/navigation-header
```
...
```bash
npm install -D @cyclone-ui/navigation-header
```
...
```bash
yarn add -D @cyclone-ui/navigation-header
```
...
Run `nx build navigation-header` to build the library.

### Running unit tests

Run `nx test navigation-header` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint navigation-header` to run [ESLint](https://eslint.org/) on the package.
