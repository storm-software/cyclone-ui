...
```bash
pnpm add -D @cyclone-ui/file-tree
```
...
```bash
npm install -D @cyclone-ui/file-tree
```
...
```bash
yarn add -D @cyclone-ui/file-tree
```
...
Run `nx build file-tree` to build the library.

### Running unit tests

Run `nx test file-tree` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint file-tree` to run [ESLint](https://eslint.org/) on the package.
