...
A package containing the CodeBlock component used by Storm Software
...
```bash
pnpm add -D @cyclone-ui/code-block
```
...
```bash
npm install -D @cyclone-ui/code-block
```
...
```bash
yarn add -D @cyclone-ui/code-block
```
...
Run `nx build code-block` to build the library.

### Running unit tests

Run `nx test code-block` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint code-block` to run [ESLint](https://eslint.org/) on the package.
