...
A ScrollView component meant to be shared in many other components
...
```bash
pnpm add -D @cyclone-ui/scroll-view
```
...
```bash
npm install -D @cyclone-ui/scroll-view
```
...
```bash
yarn add -D @cyclone-ui/scroll-view
```
...
Run `nx build scroll-view` to build the library.

### Running unit tests

Run `nx test scroll-view` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint scroll-view` to run [ESLint](https://eslint.org/) on the package.
