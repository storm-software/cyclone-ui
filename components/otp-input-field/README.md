...
One-Time Password (OTP) Input Form Field component for Cyclone UI
...
- [otp-input-field](#otp-input-field)
...
```bash
pnpm add -D @cyclone-ui/otp-input-field
```
...
```bash
npm install -D @cyclone-ui/otp-input-field
```
...
```bash
yarn add -D @cyclone-ui/otp-input-field
```
...
Run `nx build otp-input-field` to build the library.

### Running unit tests

Run `nx test otp-input-field` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint otp-input-field` to run [ESLint](https://eslint.org/) on the package.
