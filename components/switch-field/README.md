...
Switch Form Field component for Cyclone UI
...
```bash
pnpm add -D @cyclone-ui/switch-field
```
...
```bash
npm install -D @cyclone-ui/switch-field
```
...
```bash
yarn add -D @cyclone-ui/switch-field
```
...
Run `nx build switch-field` to build the library.

### Running unit tests

Run `nx test switch-field` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint switch-field` to run [ESLint](https://eslint.org/) on the package.
