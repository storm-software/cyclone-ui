...
A `DisplayText` component used to render the body text of a page. It is a simple component that renders the text in a way that is easy to read and understand.
...
```bash
pnpm add -D @cyclone-ui/display-text
```
...
```bash
npm install -D @cyclone-ui/display-text
```
...
```bash
yarn add -D @cyclone-ui/display-text
```
...
Run `nx build display-text` to build the library.

### Running unit tests

Run `nx test display-text` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint display-text` to run [ESLint](https://eslint.org/) on the package.
