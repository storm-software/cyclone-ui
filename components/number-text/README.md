...
A NumberText component meant to be shared in many other components
...
```bash
pnpm add -D @cyclone-ui/number-text
```
...
```bash
npm install -D @cyclone-ui/number-text
```
...
```bash
yarn add -D @cyclone-ui/number-text
```
...
Run `nx build number-text` to build the library.

### Running unit tests

Run `nx test number-text` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint number-text` to run [ESLint](https://eslint.org/) on the package.
