...
A wrapper component that initially covers its children with a foreground-colored
rectangle, then slides the rectangle left or right to reveal the content.
...
- [Redacted Animation](#redacted-animation)
...
```bash
pnpm add -D @cyclone-ui/redacted-animation
```
...
```bash
npm install -D @cyclone-ui/redacted-animation
```
...
```bash
yarn add -D @cyclone-ui/redacted-animation
```
...
Run `nx build redacted-animation` to build the library.

### Running unit tests

Run `nx test redacted-animation` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint redacted-animation` to run [ESLint](https://eslint.org/) on the package.
