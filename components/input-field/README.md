...
Input Form Field component for Cyclone UI
...
```bash
pnpm add -D @cyclone-ui/input-field
```
...
```bash
npm install -D @cyclone-ui/input-field
```
...
```bash
yarn add -D @cyclone-ui/input-field
```
...
Run `nx build input-field` to build the library.

### Running unit tests

Run `nx test input-field` to execute the unit tests via [Jest](https://jestjs.io).

### Linting

Run `nx lint input-field` to run [ESLint](https://eslint.org/) on the package.
