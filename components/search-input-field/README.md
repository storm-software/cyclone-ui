...
Search Input Form Field component for Cyclone UI
...
```bash
pnpm add -D @cyclone-ui/search-input-field
```
...
```bash
npm install -D @cyclone-ui/search-input-field
```
...
```bash
yarn add -D @cyclone-ui/search-input-field
```
...
Run `nx build search-input-field` to build the library.

### Running unit tests

Run `nx test search-input-field` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint search-input-field` to run [ESLint](https://eslint.org/) on the package.
