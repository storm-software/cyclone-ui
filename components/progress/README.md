...
```bash
pnpm add -D @cyclone-ui/progress
```
...
```bash
npm install -D @cyclone-ui/progress
```
...
```bash
yarn add -D @cyclone-ui/progress
```
...
Run `nx build progress` to build the library.

### Running unit tests

Run `nx test progress` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint progress` to run [ESLint](https://eslint.org/) on the package.
