...
A decorative, seeded square-grid transition between sections. The default layout has 30 columns and six rows of square cells, with a solid lower seam. A custom `height` changes the row count without stretching the cells.
...
- `animate="never"`: a stable, still glitch border.
- `animate="always"`: continuously reveals and removes blocks.
- `animate="scroll"` (default): reveals blocks as the separator crosses the viewport; scrolling back reverses the reveal. Supports page and nested-container scrolling.
- `seed`: reproducible pattern, default `27`.
- `topColor="transparent"`: lets underlying artwork show through when positioned over a hero.

Web animations respect live changes to `prefers-reduced-motion`. Native platforms render a still border. The deprecated `animated` boolean maps to `always`/`never`; an explicit `animate` takes precedence.

<!-- START doctoc -->
<!-- DON'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE -->
## Table of Contents

- [Glitch Separator](#glitch-separator)
...
```bash
pnpm add -D @cyclone-ui/glitch-separator
```
...
```bash
npm install -D @cyclone-ui/glitch-separator
```
...
```bash
yarn add -D @cyclone-ui/glitch-separator
```
...
Run `nx build glitch-separator` to build the library.

### Running unit tests

Run `nx test glitch-separator` to execute the unit tests via [Vitest](https://vitest.dev).

### Linting

Run `nx lint glitch-separator` to run [ESLint](https://eslint.org/) on the package.
